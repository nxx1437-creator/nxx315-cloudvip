...
export default function SocialRow({ onSelect }) {
  const items = [
    ...
    },
    {
      ...
      label: "Discord",
      supported: true,
      bg: "hover:bg-[#5865F2]/10 hover:border-[#5865F2]/30",
      icon: (
    ...
    <div className="grid grid-cols-4 gap-2.5">
      {items.map((item) => (
        <button
          ...
          className={`flex h-12 items-center justify-center rounded-xl border border-slate-200 bg-white transition-all active:scale-95 ${item.bg}`}
        >
          {item.icon}
        </button>
      ))}
