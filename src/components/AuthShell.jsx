...
export default function AuthShell({ title, subtitle, children, footer }) {
  ...
  const pageBg = dark ? "bg-[#0B0614] text-pink-100" : "bg-[#FFF3F8] text-pink-950";
  const cardBg = dark
    ? "bg-white/[0.04] border-white/10 backdrop-blur-xl"
    : "bg-white/80 border-pink-200 backdrop-blur-xl";
    ...
    <div className={`relative min-h-screen w-full font-[Be_Vietnam_Pro] transition-colors duration-500 ${pageBg}`}>
      ...
      <div className="relative mx-auto flex min-h-screen max-w-md flex-col items-center justify-center px-5 py-16">
        <div className="mb-7 flex flex-col items-center text-center">
          <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-pink-400 to-fuchsia-600 shadow-lg shadow-pink-500/30">
            <Coins size={22} className="text-white" />
          </div>
          <span className={`font-display mt-3 text-2xl font-bold ${dark ? "text-pink-50" : "text-pink-950"}`}>
            CloudVIP
          </span>
        ...
        <div className={`w-full rounded-3xl border p-7 sm:p-8 ${cardBg}`}>
          <h1 className={`font-display text-2xl font-bold ${dark ? "text-pink-50" : "text-pink-950"}`}>
            {title}
          </h1>
