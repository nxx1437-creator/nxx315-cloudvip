...
export default function Store() {
  ...
  const loadData = async () => {
    ...
    const [packageResult, redemptionResult, orderResult] =
      await Promise.all([
        supabase
    ...
    if (redemptionResult.error) {
      console.error("Redemption history error:", redemptionResult.error);
    }

    if (orderResult.error) {
      console.error("Roblox order history error:", orderResult.error);
    }

    const redemptionHistory = (redemptionResult.data || []).map((item) => ({
      ...item,
      history_type: "redemption",
      history_id: item.id,
    }));

    const orderHistory = (orderResult.data || []).map((item) => ({
      ...item,
      history_type: "order",
      history_id: item.id,
    }));

    const mergedHistory = [...redemptionHistory, ...orderHistory].sort(
      (a, b) =>
  ...
  const refreshHistory = async () => {
    ...
    const [redemptionResult, orderResult] = await Promise.all([
      supabase
        .from("redemption_orders")
        .select("*")
        .eq("user_id", userId)
...
function getHistoryImage(order) {
  ...
  if (order?.history_type === "order") {
    const robux = Number(order?.robux || 0);
    const known = {
      40: "roblox-40.png",
      80: "roblox-80.png",
      400: "roblox-400.png",
    ...
    if (known[robux]) {
      return `https://rwglwovohbyqmbbzdvdj.supabase.co/storage/v1/object/public/game_logos/${known[robux]}`;
    }
  }
...
function getHistoryName(order) {
  if (order?.history_type === "order") {
    if (order.robux) return `${Number(order.robux).toLocaleString("vi-VN")} Robux`;
    return order.package_name || "Đơn Roblox";
  }
...
function getHistoryAmount(order) {
  if (order?.history_type === "order") {
    const amount = order?.amount ?? order?.price_vnd;
    return amount != null
    ...
    <section className="mt-8">
      <div className="mb-3 flex items-end justify-between gap-3">
        ...
        {history.length > 0 && (
          <button
            ...
            className="rounded-xl bg-sky-50 px-3.5 py-2 text-xs font-black text-sky-600 transition hover:bg-sky-100"
          >
            Xem tất cả <ChevronRight size={14} className="ml-1 inline" />
          </button>
        )}
      </div>
      ...
      {!history.length ? (
        <div className="rounded-[24px] border border-dashed border-slate-200 bg-white px-5 py-10 text-center">
          ...
          <p className="mt-1 text-xs text-slate-400">
            Đơn hàng của bạn sẽ hiện ở đây.
          </p>
        </div>
      ) : (
        <div className="space-y-3">
          {history.slice(0, 5).map((order) => {
            const status = getHistoryStatus(order);
            const image = getHistoryImage(order);
            ...
            return (
              <button
                ...
                className="w-full rounded-[20px] border border-slate-100 bg-white p-3.5 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-sky-200 hover:shadow-md"
              >
                <div className="flex items-center gap-3">
                  <div className="h-14 w-14 shrink-0 overflow-hidden rounded-2xl bg-sky-50 ring-1 ring-slate-100">
                    {image ? (
                      <img
                        src={image}
                        alt={getHistoryName(order)}
                        className="h-full w-full object-cover"
                        onError={(event) => {
                          event.currentTarget.style.display = "none";
                        }}
