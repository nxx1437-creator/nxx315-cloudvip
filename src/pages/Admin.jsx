...
const TABS = [
  ...
  { key: "tasks", label: "Nhiệm vụ", icon: ListChecks, desc: "Cấu hình nhiệm vụ & thưởng" },
  { key: "packages", label: "Gói Robux", icon: Gift, desc: "Quản lý cửa hàng" },
  { key: "users", label: "Người dùng", icon: Users, desc: "Quản lý tài khoản & Coin" },
];
          ...
          <div key={o.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm hover:shadow-md transition">
            ...
            {o.receive_method && (
              <div className="mt-3 rounded-xl bg-slate-50 p-2.5 text-xs text-slate-600">
                Nhận qua <span className="font-bold">{o.receive_method === "zalo" ? "Zalo" : "Discord"}</span>: {o.contact_value}
              </div>
            )}
            ...
            {o.coins_refunded > 0 && (
              <p className="mt-2 text-xs font-semibold text-emerald-600">💰 Đã hoàn {o.coins_refunded.toLocaleString("vi-VN")} Coin</p>
            )}
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-bold text-slate-900">Quản lý Nhiệm vụ</h2>
        <button
          ...
          <div key={t.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 grid grid-cols-2 gap-3">
              <label className="block">
                <span className="text-[11px] font-bold uppercase text-slate-400">Coin/lượt</span>
                <input
                  type="number"
              ...
              <label className="block">
                <span className="text-[11px] font-bold uppercase text-slate-400">Giới hạn/ngày</span>
                <input
                  type="number"
                  value={t.daily_limit}
            ...
            >
              {savingId === t.id ? <Loader2 size={16} className="animate-spin mx-auto" /> : "Lưu cài đặt"}
            </button>
          </div>
        ))}
      </div>
...
function PackagesTab() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-bold text-slate-900">Quản lý Gói Robux</h2>
        <span className="rounded-full bg-blue-50 px-4 py-1.5 text-sm font-semibold text-blue-600">{packages.length} Gói đang có</span>
      </div>
        ...
        {packages.map((p) => (
          <div key={p.id} className="group relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition hover:shadow-lg">
            ...
            <div className="relative">
              ...
              <p className="text-xs font-medium text-slate-400">Phiên bản: {p.version}</p>

              <div className="mt-5 flex items-end gap-1">
                <span className="text-3xl font-extrabold text-blue-600">{Number(p.coin_cost).toLocaleString("vi-VN")}</span>
                <span className="mb-1 text-sm font-medium text-slate-400">Coin</span>
              </div>

              <div className="mt-5 space-y-3">
                <label className="block">
                  <span className="text-[11px] font-bold uppercase text-slate-400">Giá Coin</span>
                  <input
                    type="number"
                    value={p.coin_cost}
                ...
                <label className="block">
                  <span className="text-[11px] font-bold uppercase text-slate-400">Giá gốc (hiển thị)</span>
                  <input
