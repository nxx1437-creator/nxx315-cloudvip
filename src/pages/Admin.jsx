...
function MarketingTab() {
  ...
  const handleApprove = async (video) => {
    ...
    // Cộng MARKETING Coin (KHÔNG cộng vào Main coin!)
    const { data: userData } = await supabase.from("profiles").select("marketing_coins").eq("id", video.user_id).single();
    if (userData) {
      await supabase.from("profiles").update({ marketing_coins: (userData.marketing_coins || 0) + coin }).eq("id", video.user_id);
    }
    setSavingId(null);
    await fetchVideos();
  };
