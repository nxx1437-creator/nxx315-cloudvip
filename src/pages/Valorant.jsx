...
const VALORANT_BANNER = `${SUPABASE_STORAGE}/valorant-banner.png`;
...
export default function Valorant() {
  ...
  const continueToPayment = async () => {
    if (!selectedPackage) {
      alert("Vui lòng chọn gói VP.");
      return;
    }
    if (!riotId.trim()) {
      alert("Vui lòng nhập Riot ID.");
      return;
    }

    // Validate Riot ID: dạng "PlayerName#TAG"
    const cleanRiotId = riotId.trim();
    const riotIdRegex = /^.{3,20}#.{3,5}$/;
    if (!riotIdRegex.test(cleanRiotId)) {
      alert(
        "Riot ID không hợp lệ.\n\nĐịnh dạng đúng: PlayerName#TAG\nVí dụ: SKT1#1234"
      );
      return;
    }
    ...
    try {
      ...
      console.error("Create Valorant order:", error);
  ...
  const handleCardPayment = async () => {
    ...
    try {
      ...
      console.error("Valorant card payment error:", error);
      setCardResult({
        status: "error",
      ...
      <div className="relative -mt-10 px-5 pb-5">
        <div className="flex items-end gap-3.5">
          <div className="relative shrink-0">
            <div className="absolute -inset-0.5 rounded-2xl bg-gradient-to-br from-red-500 to-rose-600 opacity-60 blur-md" />
            <div className="relative h-20 w-20 overflow-hidden rounded-2xl border-4 border-white bg-red-50 shadow-lg sm:h-24 sm:w-24">
              <img
                src={VALORANT_LOGO}
                alt="VALORANT"
                className="h-full w-full object-cover"
          ...
          <div className="min-w-0 flex-1 pb-1">
            ...
            <p className="mt-0.5 text-xs text-slate-500 sm:text-sm">
              Nạp VP chính hãng · Tự động 24/7
            </p>
          </div>
        </div>

        <div className="mt-4 grid grid-cols-3 gap-2">
          <StatPill icon={Zap} label="Tức thì" color="red" />
    ...
    <div className="space-y-4">
      <div className="flex items-start gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-4">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-sm font-bold text-amber-900">
            Kiểm tra kỹ Riot ID trước khi nạp
          </p>
          <p className="mt-1 text-xs leading-5 text-amber-800">
            VP sẽ nạp vào Riot ID đã xác nhận. Sai ID không hoàn tiền.
          </p>
        </div>
      </div>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="mb-4 flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-red-500 to-rose-600 text-white shadow-md shadow-red-200">
            <User size={20} />
          </div>
          <div>
            ...
            <p className="text-xs text-slate-500">
              Nhập Riot ID để nhận VP
            </p>
          </div>
        </div>

        <label className="mb-2 block text-[11px] font-bold uppercase tracking-widest text-red-500">
          Riot ID
        </label>
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Ví dụ: SKT1#1234"
            className="w-full rounded-xl border border-slate-200 bg-slate-50 py-4 pl-11 pr-4 text-sm font-bold text-slate-800 outline-none transition placeholder:font-medium placeholder:text-slate-300 focus:border-red-400 focus:bg-white focus:ring-4 focus:ring-red-100"
          />
        </div>

        <p className="mt-2 text-[11px] text-slate-400">
          Định dạng: <b className="text-slate-600">PlayerName#TAG</b>
        </p>
      </section>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="mb-4 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-red-500 to-rose-600 text-white shadow-md shadow-red-200">
              <Target size={16} />
            </div>
            <div>
              <h2 className="text-base font-black text-slate-900">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <button
          onClick={onContinue}
          disabled={!selectedPackage || !riotId.trim() || creatingOrder || !agreedTerms}
          className="mt-3 group relative flex h-14 w-full items-center justify-center gap-2 overflow-hidden rounded-2xl bg-gradient-to-r from-red-500 to-rose-600 text-sm font-black text-white shadow-lg shadow-red-300/50 transition hover:shadow-red-400/60 disabled:cursor-not-allowed disabled:opacity-40 disabled:shadow-none"
        >
        ...
        {selectedPackage && !riotId.trim() && (
          <p className="mt-2 text-center text-xs text-slate-400">
            Nhập Riot ID để tiếp tục
          </p>
        )}
      </section>
    </div>
  ...
  return (
    <button
      ...
      className={`group relative overflow-hidden rounded-2xl border-2 bg-white p-3 text-left transition-all duration-200 ${
        active
          ? "border-red-500 bg-red-50/40 shadow-lg shadow-red-200/60 -translate-y-1"
    ...
    >
      ...
      <div className="relative text-center">
        ...
        <p className="mt-0.5 text-[10px] font-bold uppercase tracking-widest text-red-500">
          VP
        </p>
...
function PaymentStep({
  ...
  if (!order) {
    return (
      <div className="rounded-2xl border border-slate-200 bg-white p-8 text-center shadow-sm">
        ...
        <button
          onClick={onBack}
          className="mt-5 rounded-xl bg-gradient-to-r from-red-500 to-rose-600 px-5 py-3 text-sm font-bold text-white shadow-md shadow-red-200"
        >
          Quay lại
        </button>
      </div>
  ...
  const transferContent = `NAP VALORANT ${order.order_code} ${order.riot_id} ${order.vp}VP`;
  const coinBalance = Number(profile?.coins || 0);
  const requiredCoins = Number(order.amount || 0);
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-4">
      <div className="flex items-center gap-3">
        <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-gradient-to-br from-red-500 to-rose-600 shadow-md shadow-red-200">
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-100 bg-gradient-to-br from-red-50 to-rose-50 p-5">
          ...
          <div className="mt-3 inline-flex items-center gap-1.5 rounded-full border border-red-200 bg-white px-3 py-1.5 text-[11px] font-bold text-slate-600">
            Mã đơn:{" "}
            <span className="font-mono text-red-600">{order.order_code}</span>
          </div>
        </div>
        ...
        <div className="space-y-4 p-4 sm:p-5">
          ...
          {method === "coin" && (
            <div className="space-y-3">
              ...
              <button
                ...
                className="group relative h-12 w-full overflow-hidden rounded-xl bg-gradient-to-r from-red-500 to-rose-600 text-sm font-bold text-white shadow-md shadow-red-200 transition hover:shadow-red-300/60 disabled:cursor-not-allowed disabled:opacity-40 disabled:shadow-none"
              >
                <div className="absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-white/30 to-transparent transition-transform duration-700 group-hover:translate-x-full" />
          ...
          {method === "bank" && (
            <div className="space-y-3">
              ...
              {order.status === "paid" ? (
                ...
                <button
                  ...
                  className="group relative h-12 w-full overflow-hidden rounded-xl bg-gradient-to-r from-red-500 to-rose-600 text-sm font-bold text-white shadow-md shadow-red-200 transition hover:shadow-red-300/60"
                >
                  <div className="absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-white/30 to-transparent transition-transform duration-700 group-hover:translate-x-full" />
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-3 gap-1.5 sm:grid-cols-5">
                {cardTypes.map((type) => (
                  ...
                  >
                    ...
                    <p className="mt-0.5 text-xs font-black text-red-500">
                      {cardDiscounts[type]}%
                    </p>
                  </div>
                ))}
                ...
                return (
                  ...
                  >
                    <div className="mb-3 flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <span className="flex h-6 w-6 items-center justify-center rounded-lg bg-gradient-to-br from-red-500 to-rose-600 text-[11px] font-black text-white shadow-sm">
                          {index + 1}
                        </span>
                    ...
                    <div className="space-y-3">
                      <div>
                        <label className="mb-1.5 block text-[11px] font-bold uppercase tracking-widest text-red-500">
                          Loại thẻ
                        </label>
                        <select
                          value={card.type}
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          ...
                          <input
                            ...
                            className="h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm font-semibold text-slate-700 outline-none transition placeholder:font-normal placeholder:text-slate-300 focus:border-red-400 focus:ring-2 focus:ring-red-100"
                          />
                        </div>
                        <div>
                          <label className="mb-1.5 block text-[11px] font-bold uppercase tracking-widest text-red-500">
              ...
              <button
                onClick={addCard}
                className="h-11 w-full rounded-xl border-2 border-dashed border-red-200 text-sm font-bold text-red-500 transition hover:border-red-400 hover:bg-red-50"
              >
                + Thêm thẻ
              </button>
              ...
              <button
                ...
                className="group relative flex h-12 w-full items-center justify-center gap-2 overflow-hidden rounded-xl bg-gradient-to-r from-red-500 to-rose-600 text-sm font-bold text-white shadow-md shadow-red-200 transition hover:shadow-red-300/60 disabled:opacity-50"
              >
                <div className="absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-white/30 to-transparent transition-transform duration-700 group-hover:translate-x-full" />
                {cardChecking ? (
                  <>
      ...
      <div className="flex min-w-0 items-center gap-2">
        ...
        {(copy || copyValue) && (
          <button
            type="button"
            onClick={handleCopy}
            className="shrink-0 rounded-lg p-1.5 text-slate-400 transition hover:bg-slate-100 hover:text-red-500"
            title="Sao chép"
          >
            <Copy size={14} />
          </button>
