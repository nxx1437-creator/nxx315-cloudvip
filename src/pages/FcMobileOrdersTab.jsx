...
export default function FcMobileOrdersTab() {
  ...
  const fetchOrders = async (silent = false) => {
    ...
    try {
      const { data, error: fetchError } = await supabase
        .from("orders")
        .select("*")
        .not("fco_character_id", "is", null)
        .order("created_at", { ascending: false });

      if (fetchError) throw fetchError;
      setOrders(data || []);
    } catch (err) {
      console.error("FETCH FC MOBILE ORDERS ERROR:", err);
      setError(`Không thể tải danh sách đơn FC Mobile: ${err?.message || "Lỗi"}`);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel("admin-fc-mobile-orders")
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "orders" },
        (payload) => {
          const row = payload.new || payload.old;
          if (row?.fco_character_id == null) return;
  ...
  const updateOrderStatus = async (order, expected, next, note) => {
    ...
    try {
      ...
      if (!data) {
        setError(`Không cập nhật được đơn ${order.order_code || order.id}.`);
      ...
      setError(`Lỗi cập nhật: ${err?.message || "Không xác định"}`);
      return false;
    } finally {
      setUpdatingId(null);
    }
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-16">
        ...
        <span className="ml-2">Đang tải đơn FC Mobile...</span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <h2 className="text-xl font-bold">Đơn FC Mobile VN</h2>
          <p className="text-sm text-gray-500">
            Quản lý đơn nạp FC Mobile.
          </p>
        </div>
      ...
      <div className="flex flex-col gap-3 md:flex-row">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Tìm mã đơn, Character Name..."
      ...
      {filteredOrders.length === 0 ? (
        <div className="rounded-2xl border bg-white py-16 text-center text-gray-500">
          Không có đơn FC Mobile phù hợp.
        </div>
      ) : (
                    ...
                    <div className="grid gap-x-8 gap-y-1 text-sm text-gray-600 md:grid-cols-2">
                      ...
                      <div>
                        ...
                        <span className="font-semibold text-gray-900">
                          {Number(order.fco_fc || 0).toLocaleString("vi-VN")} FC
                        </span>
                      </div>
                      <div>
                        Số tiền:{" "}
                      ...
                      <>
                        ...
                        <button
                          onClick={() => {
                            ...
                            if (reason) {
                              updateOrderStatus(order, "paid", "rejected", reason);
                            }
                          }}
                          disabled={updating}
