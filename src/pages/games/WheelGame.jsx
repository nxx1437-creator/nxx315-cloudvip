              ...
              return (
                ...
                >
                  <div 
                    ...
                    style={{ transform: "translateX(-50%) translateY(-135px)" }}
                  >
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[#EAF3FC] text-[#111827]">
      ...
      <header className="flex items-center px-4 pb-3 pt-4">
        <button
          onClick={() => navigate(-1)}
          className="flex h-[46px] w-[46px] items-center justify-center rounded-full bg-white text-[#374151] shadow-[0_3px_12px_rgba(0,0,0,0.05)] active:scale-95"
        >
      ...
      <section className="relative z-10 -mt-1 min-h-[420px] rounded-t-[32px] bg-white px-4 pb-10 pt-6 shadow-[0_-7px_25px_rgba(0,0,0,0.035)]">
        ...
        <div className="scrollbar-none mt-4 flex gap-3 overflow-x-auto pb-3">
          {SEGMENTS.map((item) => (
            <div key={item.id} className="flex w-[138px] shrink-0 flex-col items-center rounded-[21px] border border-[#E3E6EB] bg-white px-3 pb-4 pt-3 text-center">
              <div className={`mb-3 h-[4px] w-[48px] rounded-full ${item.isBigWin ? "bg-[#F2A900]" : "bg-[#3478F6]"}`} />
      ...
      {result !== null && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
          <div className="relative w-full max-w-[330px] rounded-[30px] bg-white p-6 text-center shadow-2xl">
            ...
            <button
              onClick={closeResult}
              className="absolute right-4 top-4 z-20 flex h-8 w-8 items-center justify-center rounded-full bg-[#F5F7FB] text-[#9CA3AF]"
            >
              <X size={16} />
            </button>
            ...
            <button
              onClick={closeResult}
              className="relative mt-6 w-full rounded-[16px] bg-gradient-to-r from-[#3478F6] to-[#0878C9] py-[13px] text-[14px] font-black text-white shadow-md active:scale-[0.98]"
            >
              Tuyệt vời
            </button>
          </div>
