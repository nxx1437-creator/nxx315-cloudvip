...
// Derived card data used by the card-payment UI.
const cardTypes = Object.keys(CARD_TYPES);
const cardDiscounts = Object.fromEntries(
  cardTypes.map((type) => {
    const first = CARD_TYPES[type]?.[0];
    const discount = first?.value
      ? Math.round((1 - Number(first.received || 0) / Number(first.value)) * 100)
      : 0;
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
        ...
        <div className="relative">
          ...
          <input
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter" && !creatingOrder) onContinue();
            }}
            inputMode="numeric"
