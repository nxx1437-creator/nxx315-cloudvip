...
const mergeOrder = (oldOrder, newOrder) => {
  ...
  const updateSelectedFromHistory = (items) => {
    ...
    const found = items.find((item) => {
      ...
      if (selectedSource) return item._source === selectedSource;
      return true;
    });
  ...
  const applyRealtimeChange = (source, payload) => {
    ...
    setHistory((current) => {
      ...
      if (index === -1) {
        updated = [{ ...newRow, _source: source }, ...current];
      } else {
        updated = [...current];
        ...
        <main className="mx-auto max-w-2xl px-4 py-5">
          ...
          <div className="overflow-hidden rounded-3xl bg-white shadow-sm">
            ...
            <div className="space-y-4 p-5">
              ...
              {order.ff_uid && (
                <div className="rounded-2xl bg-slate-50 p-4">
                  <p className="text-xs text-slate-400">UID Free Fire</p>
                  <p className="mt-1 font-semibold text-slate-900">
                    {order.ff_uid}
                  </p>
                </div>
              ...
              {order.note && order.status === "delivered" && (
                <div className="rounded-2xl bg-slate-50 p-4">
