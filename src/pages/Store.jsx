            ...
            {history.length === 0 ? (
              ...
              return (
                <div key={order.id} className="rounded-2xl border border-slate-100 p-4">
                  ...
                  {order.status === "pending" && <span className="text-amber-500 text-xs">⏳ Đang xử lý</span>}
                  {order.status === "delivered" && <span className="text-emerald-500 text-xs">✅ Đã giao</span>}
                  {order.status === "rejected" && <span className="text-rose-500 text-xs">❌ Từ chối</span>}
                </div>
              );
            })}
          </div>
