...
function getHistoryAmount(order) {
  const coins = order?.coin_cost ?? order?.coins ?? order?.coin_amount;
  if (coins != null) return `${Number(coins).toLocaleString('vi-VN')} xu`;

  const money = order?.price_vnd ?? order?.amount_vnd ?? order?.amount;
...
function detectGame(packageId) {
  ...
  if (pid.startsWith('vng-') || pid.startsWith('card-') || pid.includes('roblox') || pid.includes('robux')) return 'roblox';
...
function getHistoryImage(order) {
  ...
  if (packageId.startsWith('pubg-') || packageId.includes('pubg')) return getImageUrl('pubg-mobile-vn.png');
  if (packageId.startsWith('ff-') || packageId.includes('freefire')) return getImageUrl('free-fire.png');
    ...
    const load = async () => {
      try {
        ...
        const merged = [
          ...
          ...(redemptionResult.data || []).map((o) => ({ ...o, historySource: 'redemption_orders' })),
        ]
          ...
          {history.map((order) => {
            ...
            return (
              <button
                ...
                type="button"
                onClick={() => navigate(`/history/order/${order.id}?source=${order.historySource}`)}
                className="w-full rounded-2xl border border-gray-100 bg-white p-3.5 text-left shadow-sm transition hover:border-orange-100 hover:shadow-md"
              >
                <div className="flex items-center gap-3">
                  <div className="flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-xl border border-gray-100 bg-gray-50">
                    <img src={getHistoryImage(order)} alt="" className="h-full w-full object-contain p-1.5" />
                  </div>
                  <div className="min-w-0 flex-1">
                  ...
                  <ChevronRight size={17} className="shrink-0 text-orange-300" />
                </div>
              </button>
            );
          })}
