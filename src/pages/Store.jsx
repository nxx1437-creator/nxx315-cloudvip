...
function getHistoryStatus(status) {
  ...
  if (['completed', 'success', 'paid', 'done'].includes(key)) {
    return {
      label: 'Hoàn thành',
  ...
  if (['cancelled', 'canceled', 'failed', 'rejected'].includes(key)) {
    return {
      label: key === 'failed' || key === 'rejected' ? 'Thất bại' : 'Đã hủy',
        ...
        {loading ? (
          <div className="space-y-2">
            {[1, 2, 3].map((item) => (
              <div
                key={item}
                className="h-[78px] rounded-2xl bg-white border border-gray-100 animate-pulse"
              />
            ))}
          </div>
        ) : history.length === 0 ? (
          <div className="rounded-2xl bg-white border border-gray-100 px-5 py-9 text-center shadow-sm">
            ...
            {history.map((order) => {
              ...
              return (
                <button
                  ...
                  onClick={() =>
                    navigate(
                      `/history/order/${encodeURIComponent(order.id)}`
                    )
                  }
                  className="w-full rounded-2xl bg-white border border-gray-100 p-3.5 text-left shadow-sm transition hover:border-blue-100 hover:shadow-md active:scale-[0.995]"
                >
                  <div className="flex items-center gap-3">
                    <div className="w-11 h-11 rounded-xl bg-blue-50 flex items-center justify-center shrink-0">
                      <Gift size={19} className="text-blue-500" />
                    </div>

                    <div className="min-w-0 flex-1">
                      <p className="truncate text-sm font-black text-gray-900">
                        {order.package_name || order.product_name || 'Giao dịch'}
                      </p>
