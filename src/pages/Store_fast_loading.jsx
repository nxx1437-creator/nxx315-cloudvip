...
function GameImage({ src, alt }) {
  ...
  return (
    <div className="relative w-full h-full overflow-hidden rounded-xl bg-gray-50">
      {/* Skeleton chỉ hiện tối đa 350ms */}
      {showSkeleton && !loaded && !error && (
        <div className="absolute inset-0 z-20 rounded-xl bg-gray-100 animate-pulse" />
      )}

      {/* Fallback luôn có sẵn, không để ô ảnh trống */}
      {!loaded && (
        <div className="absolute inset-0 z-0 flex flex-col items-center justify-center bg-gray-50 text-gray-300">
          <Gamepad2 size={34} strokeWidth={1.5} />
          <span className="text-[9px] mt-1">
            {error ? 'Đang cập nhật' : 'Game'}
          </span>
        </div>
      )}
...
function Recommended({ navigate, loading = false }) {
        ...
        >
          {loading
            ? recommended.map((game) => (
                <div
                  key={`recommended-skeleton-${game.id}`}
                  className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm"
                >
                  <div className="aspect-[1.25/1] bg-gray-100 animate-pulse" />
                  <div className="p-3">
      ...
      <div className="max-w-5xl mx-auto">
        ...
        {loading ? (
          ...
          >
            {games.map((game) => (
              <div
                key={`game-skeleton-${game.id}`}
                className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm"
              >
                <div className="aspect-square bg-gray-100 animate-pulse" />
                <div className="p-3">
                  <div className="h-3 w-24 bg-gray-100 rounded animate-pulse" />
                  <div className="h-8 w-full bg-gray-100 rounded-xl mt-2 animate-pulse" />
                </div>
              </div>
            ))}
          </div>
...
function getHistoryImage(order) {
  ...
  // Roblox orders thường có package_id như vng-40, vng-80, card-400...
