...
const FCM_BANNER = `${SUPABASE_STORAGE}/fc-mobile-banner.png`;
const FCM_LOGO = `${SUPABASE_STORAGE}/fc-mobile.png`;
...
export default function FcMobile() {
  ...
  const continueToPayment = async () => {
    ...
    if (!characterId.trim()) {
      alert("Vui lòng nhập Character Name.");
      return;
    }
    ...
    try {
      ...
      console.error("Create FC Mobile order:", error);
      ...
      if (msg.includes("Vui lòng chờ")) {
        ...
        alert(`🚫 ${msg}`);
      } else if (msg.includes("Bạn đã có đơn chưa thanh toán")) {
        alert(`⚠️ ${msg}`);
      } else {
        alert(msg);
      }
  ...
  const handleCardPayment = async () => {
    ...
    try {
      ...
      console.error("FC Mobile card payment error:", error);
      setCardResult({
        status: "error",
    ...
    <div className="space-y-4">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <button
          type="button"
          onClick={onShowGuide}
          className="mt-3 inline-flex items-center gap-1.5 text-xs font-semibold text-green-600 transition hover:text-green-700 hover:underline"
        >
          <Info size={13} />
          Hướng dẫn lấy Character Name
        </button>
      </section>
...
function PaymentStep({
  ...
  const transferContent = `NAP FCMOBILE ${order.order_code} ${order.fco_character_id} ${order.fco_fc}FC`;
  const coinBalance = Number(profile?.coins || 0);
  const requiredCoins = Number(order.amount || 0);
...
function GuideModal({ onClose }) {
  const GUIDE_IMAGES = [
    {
      step: 1,
      url: `${SUPABASE_STORAGE}/fc-mobile-guide-1.png`,
      title: "Vào giao diện game FC Mobile VN",
    },
    {
      step: 2,
      url: `${SUPABASE_STORAGE}/fc-mobile-guide-2.png`,
      title: "Vào Hồ sơ → Copy Character Name",
    },
  ];
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 backdrop-blur-sm sm:items-center sm:p-4">
      <div className="relative flex max-h-[90vh] w-full max-w-md flex-col overflow-hidden rounded-t-3xl bg-white shadow-2xl sm:rounded-2xl">
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <h3 className="mt-1 text-base font-black text-slate-900">
                Lấy Character Name FC Mobile VN
              </h3>
        ...
        <div className="flex-1 space-y-4 overflow-y-auto p-5">
          {GUIDE_IMAGES.map((item) => (
            <div key={item.step}>
              <div className="mb-2 flex items-center gap-2.5">
                <span className="flex h-7 w-7 items-center justify-center rounded-xl bg-gradient-to-br from-green-400 to-emerald-600 text-[11px] font-black text-white shadow-md shadow-green-200">
                  {item.step}
                </span>
              ...
              <div className="overflow-hidden rounded-xl border border-slate-200 bg-slate-50">
                <img
                  ...
                  onError={(e) => {
                    e.currentTarget.style.display = "none";
                    e.currentTarget.parentElement.innerHTML =
                      '<div class="flex h-32 items-center justify-center text-xs text-slate-400">Ảnh hướng dẫn đang cập nhật</div>';
                  }}
                />
              </div>
            </div>
          ...
          <div className="rounded-xl border border-amber-200 bg-amber-50 p-3">
            ...
            <p className="mt-1 text-[11px] leading-5 text-amber-700">
              Character Name phải nhập <b>chính xác</b> — phân biệt chữ
              hoa/thường. Sai tên sẽ không được hoàn tiền.
            </p>
          </div>
        </div>
        ...
        <div className="border-t border-slate-100 p-5">
          <button
            onClick={onClose}
            className="h-12 w-full rounded-xl bg-gradient-to-r from-green-500 to-emerald-600 text-sm font-black text-white shadow-md shadow-green-200 transition hover:shadow-green-300/60"
          >
            Đã hiểu
          </button>
        </div>
