...
// AI chính + AI dự phòng.
// Có thể đổi endpoint bằng VITE_AI_SUPPORT_PRIMARY_URL và VITE_AI_SUPPORT_FALLBACK_URL.
// AI dự phòng nên là một Supabase Edge Function khác (ví dụ: ai-support-fallback)
// để giữ API key ở phía server, không đưa key AI vào frontend.
const AI_ENDPOINTS = [
  {
    name: "AI chính",
    url:
      import.meta.env.VITE_AI_SUPPORT_PRIMARY_URL ||
      "https://rwglwovohbyqmbbzdvdj.supabase.co/functions/v1/ai-support-reply",
  },
  {
    name: "AI dự phòng",
    url:
      import.meta.env.VITE_AI_SUPPORT_FALLBACK_URL ||
      "https://rwglwovohbyqmbbzdvdj.supabase.co/functions/v1/ai-support-fallback",
  },
];
  ...
  return (
    <div className="min-h-[calc(100vh-72px)] bg-[#f8f8f8]">
      <div className="sticky top-0 z-20 flex items-center justify-between border-b border-black/[0.06] bg-white/95 px-4 py-3 backdrop-blur-xl">
        ...
        <h1 className="text-[16px] font-extrabold tracking-[-0.02em] text-[#161823]">Bộ phận Hỗ trợ</h1>
      ...
      <div className="px-4 pb-4">
        <button
          ...
          className="flex w-full items-center justify-between rounded-[16px] border border-black/[0.05] bg-white px-4 py-4 shadow-[0_1px_3px_rgba(0,0,0,0.04)] transition hover:shadow-[0_8px_25px_rgba(0,0,0,0.07)] active:scale-[0.98]"
        >
    ...
    try {
      ...
      if (conv.status === "ai") {
        ...
        try {
          ...
          // Tự động thử AI chính -> nếu lỗi/timeout thì chuyển sang AI dự phòng.
          // Không đổi conversation/status nên người dùng không bị gián đoạn.
          let response = null;
          let lastError = null;
          let usedAI = "";

          for (let i = 0; i < AI_ENDPOINTS.length; i += 1) {
            const ai = AI_ENDPOINTS[i];
            setAiFallback(i > 0);
            ...
            try {
              ...
              response = await fetch(ai.url, {
                ...
                body: JSON.stringify({
                  ...
                  fallback_attempt: i > 0,
                  previous_ai_failed: i > 0,
                }),
              ...
              try {
                ...
                // Response không phải JSON, tiếp tục thử AI kế tiếp.
              }

              lastError = new Error(
                errData?.error || `${ai.name} trả về lỗi ${response.status}`
              );
            } catch (error) {
              lastError = error;
            }
          ...
          if (!response?.ok) {
            throw lastError || new Error("Tất cả AI đều không phản hồi");
          }

          // Có thể dùng biến này sau này để hiển thị AI nào đang trả lời.
          console.log(`[Support AI] Đang dùng: ${usedAI}`);
          setAiFallback(false);
        } catch (err) {
    ...
    <div className="relative flex h-[calc(100vh-0px)] flex-col bg-[#f8f8f8]">
      ...
      <div className="sticky top-0 z-20 flex items-center gap-3 border-b border-black/[0.06] bg-white/95 px-4 py-3 backdrop-blur-xl">
        ...
        <div className="flex min-w-0 flex-1 items-center gap-2">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[10px] font-medium text-[#8a8d93]">Trợ lý AI • Luôn sẵn sàng</p>
          </div>
        </div>
        <button
      ...
      {showHistoryMenu && (
        <>
          ...
          />
          <div className="absolute right-3 top-14 z-40 w-72 overflow-hidden rounded-[16px] border border-black/[0.06] bg-white shadow-[0_14px_40px_rgba(0,0,0,0.12)] animate-[fadeIn_0.2s_ease-out]">
                ...
                <div className="rounded-[18px] rounded-tl-[5px] border border-black/[0.05] bg-white px-4 py-3 shadow-[0_1px_4px_rgba(0,0,0,0.04)]">
                  <div className="flex items-center gap-1.5">
                    <span className="text-xs text-slate-400">
                      {aiFallback ? "AI chính đang bận, đang chuyển sang AI dự phòng…" : "Đang kiểm tra một vài chi tiết"}
                    </span>
        ...
        <div className="flex items-center gap-2">
          ...
          {input.trim() ? (
            ...
            <button
              ...
              className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-black/[0.07] bg-[#f2f2f2] text-[#161823] transition active:scale-95"
            >
  ...
  if (isUser) {
    return (
      <div className="flex justify-end animate-[slideInRight_0.3s_ease-out]">
        <div className="max-w-[82%] rounded-[18px] rounded-br-[5px] bg-[#FE2C55] px-4 py-2.5 text-white shadow-[0_3px_10px_rgba(254,44,85,0.12)]">
