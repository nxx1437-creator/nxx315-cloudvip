...
import {
  ArrowLeft, Send, Loader2, Bot, Headphones, Smile, User, CreditCard,
  Package, Bug, MoreHorizontal, ChevronRight, ChevronDown, Plus, FileText,
  RefreshCw, ArrowRight, ThumbsUp, ThumbsDown, LogIn, Lightbulb,
  Image as ImageIcon, Sparkles, HelpCircle, KeyRound, ShieldCheck, Mail,
  RotateCcw, Receipt, Search, XCircle, Truck, Globe, Smartphone,
  ImagePlus, Handshake, AlertTriangle, Camera, Edit, History, X, Menu,
  Paperclip, Mic,
} from "lucide-react";
...
const NO_CHAT_SUB_CARDS = ["Hợp tác / Đại lý", "Báo cáo vi phạm", "Câu hỏi chung"];
...
function shouldShowLoginButton(text) {
  ...
  return (
    lower.includes("mật khẩu") || lower.includes("đăng nhập") ||
    lower.includes("đăng ký") || lower.includes("xác minh") ||
    lower.includes("verify") || lower.includes("password") || lower.includes("login")
  );
}
  ...
  return (
    <div className="min-h-[calc(100vh-72px)] bg-white">
      <div className="sticky top-0 z-20 flex items-center justify-between border-b border-black/[0.06] bg-white/95 px-4 py-3 backdrop-blur-xl">
        <button onClick={() => window.history.back()} className="flex h-8 w-8 items-center justify-center text-[#161823]">
          <ArrowLeft size={22} strokeWidth={2.2} />
        </button>
        <h1 className="text-[16px] font-bold tracking-[-0.01em] text-[#161823]">Bộ phận Hỗ trợ của NXX315</h1>
      ...
      <div className="px-5 pb-6 pt-8">
        <h2 className="text-center text-[22px] font-extrabold leading-[1.3] tracking-[-0.02em] text-[#161823]">
          Chúng tôi sẵn sàng hỗ trợ!<br />Chọn loại vấn đề.
        </h2>
      </div>
        ...
        {CATEGORIES.map((cat) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-[14.5px] font-semibold text-[#161823]">{cat.label}</span>
            </button>
          );
        })}
      </div>
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              <p className="text-[14.5px] font-semibold text-[#0068FF]">Gặp nhân viên qua Zalo</p>
              <p className="mt-0.5 text-[11.5px] text-[#0068FF]/70">Hỗ trợ {SUPPORT.hours}</p>
            </div>
          </div>
  ...
  const loadData = async () => {
    ...
    try {
      const [cardsRes, faqsRes] = await Promise.all([
        supabase.from("support_sub_cards").select("*").eq("category", category).eq("is_active", true).order("order", { ascending: true }),
        supabase.from("support_faqs").select("*").eq("category", category).eq("is_active", true).order("order", { ascending: true }),
      ]);
  ...
  return (
    <div className="min-h-[calc(100vh-72px)] bg-white pb-28">
      <div className="sticky top-0 z-20 flex items-center gap-3 border-b border-black/[0.06] bg-white/95 px-4 py-3 backdrop-blur-xl">
        ...
        <h1 className="flex-1 text-[15px] font-bold tracking-[-0.01em] text-[#161823]">{cat?.label || "Hỗ trợ"}</h1>
      </div>

      <div className="px-5 pb-4 pt-6">
        <h2 className="text-[26px] font-extrabold leading-[1.2] tracking-[-0.03em] text-[#161823]">Chúng tôi có thể giúp gì?</h2>
      </div>
        ...
        <>
                ...
                {subCards.map((card) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        if (isNoChat) {
                          document.querySelector("#help-faq-section")?.scrollIntoView({ behavior: "smooth" });
                        } else {
                    ...
                    >
                      <div className="flex h-9 w-9 items-center justify-center rounded-[10px]" style={{ backgroundColor: `${card.color}22` }}>
                        <Icon size={20} style={{ color: card.color }} strokeWidth={2.2} />
                      </div>
                      <span className="text-[13.5px] font-semibold leading-tight text-[#161823]">{card.title}</span>
                    </button>
                  );
                })}
              </div>
          ...
          {faqs.length > 0 ? (
            <div id="help-faq-section" className="px-5">
              <h3 className="mb-1 text-[20px] font-extrabold tracking-[-0.02em] text-[#161823]">Câu hỏi thường gặp</h3>
              <div>
                {faqs.map((faq) => {
                  const isOpen = openId === faq.id;
                  return (
                    <div key={faq.id} className="border-b border-black/[0.06] last:border-b-0">
                      <button onClick={() => toggleFaq(faq.id)} className="flex w-full items-center gap-3 py-4 text-left transition active:opacity-60">
                        <span className="flex-1 text-[15px] font-medium leading-6 text-[#161823]">{faq.question}</span>
                        <ChevronRight size={18} className={`shrink-0 text-[#8a8d93] transition-transform duration-200 ${isOpen ? "rotate-90" : ""}`} strokeWidth={2.2} />
                      </button>
                      {isOpen && (
                        <div className="pb-4">
                          <p className="whitespace-pre-wrap text-[14px] leading-6 text-[#4a4d54]">{faq.answer}</p>
                        </div>
                      )}
                    </div>
                  );
            ...
            <div className="px-5 py-8 text-center">
              <HelpCircle size={36} className="mx-auto mb-3 text-slate-300" strokeWidth={1.8} />
              <p className="text-[14px] font-semibold text-[#161823]">Chưa có hướng dẫn cho mục này</p>
              <p className="mt-1 text-[12.5px] text-[#8a8d93]">Bạn có thể chat với AI để được hỗ trợ trực tiếp.</p>
            </div>
          )}
        </>
      )}
...
function StatusBubble({ message }) {
  ...
  return (
    <div className="px-1 py-0.5">
      ...
      >
        <Lightbulb size={16} className="shrink-0 text-slate-500" strokeWidth={2.2} />
        <span className="flex-1 truncate text-[13px] font-medium text-slate-700">{message.message}</span>
        <ChevronRight size={16} className={`shrink-0 text-slate-400 transition-transform ${expanded ? "rotate-90" : ""}`} strokeWidth={2.2} />
      </button>
      {expanded && (
...
function HistoryDrawer({ open, onClose, userId, currentConvId, onOpenConversation, onNewChat }) {
  ...
  const renderGroup = (title, items) => {
    ...
    return (
      <div className="mb-5">
        <h3 className="mb-2 px-1 text-[11px] font-bold uppercase tracking-wider text-slate-400">{title}</h3>
        <div className="space-y-0.5">
          {items.map((conv) => (
            <button
              key={conv.id}
              onClick={() => { onOpenConversation(conv); onClose(); }}
              className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left transition ${
                conv.id === currentConvId ? "bg-rose-50 text-[#FE2C55]" : "text-slate-700 hover:bg-slate-50"
              }`}
            >
              <FileText size={16} className="shrink-0 opacity-60" />
              <div className="min-w-0 flex-1">
                <p className="truncate text-[13.5px] font-medium">{conv.title || "Cuộc trò chuyện"}</p>
                <p className="truncate text-[11px] opacity-60">{formatRelativeTime(conv.created_at)}</p>
              </div>
            </button>
          ))}
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-3">
          {loading ? (
            ...
            <div className="px-4 py-10 text-center">
              <History size={36} className="mx-auto mb-3 text-slate-300" strokeWidth={1.5} />
              <p className="text-[13px] font-medium text-slate-500">Chưa có cuộc trò chuyện nào</p>
              <p className="mt-1 text-[11.5px] text-slate-400">Bắt đầu chat để lưu lịch sử</p>
            </div>
          ) : (
            <>
...
function ChatView({ conversation, user, category, onBack, onNewChat, onOpenConversation }) {
  ...
  useEffect(() => {
    const channel = supabase
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "support_messages", filter: `conversation_id=eq.${conversation.id}` }, (payload) => {
        ...
        setMessages((prev) => {
          ...
          if (msg.sender_type === "ai" || msg.sender_type === "system") {
            const withoutStatus = prev.filter((m) => m.sender_type !== "status");
      ...
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "support_messages", filter: `conversation_id=eq.${conversation.id}` }, (payload) => {
        const msg = payload.new;
        setMessages((prev) => prev.map((m) => (m.id === msg.id ? { ...m, ...msg } : m)));
      })
      .on("postgres_changes", { event: "DELETE", schema: "public", table: "support_messages", filter: `conversation_id=eq.${conversation.id}` }, (payload) => {
  ...
  const sendMessage = async (customText) => {
    ...
    try {
      ...
      setMessages((prev) => [...prev.filter((m) => m.sender_type !== "status"), userMsg]);
      setInput("");
        ...
        try {
          ...
          const { data: errMsg } = await supabase
            ...
            .insert({
              conversation_id: conv.id,
              user_id: user.id,
              message: "Xin lỗi, mình đang gặp sự cố kỹ thuật. Bạn vui lòng thử lại sau hoặc liên hệ Zalo 0865245988 để được hỗ trợ trực tiếp.",
              sender_type: "ai",
              suggestions: [],
            })
            .select()
  ...
  const hideAllSuggestions = () => {
    setHiddenSuggestionIds((prev) => {
      const allIds = messages.filter((m) => m.suggestions?.length > 0).map((m) => m.id);
          ...
          {loading ? (
            ...
            <>
              {messages.map((msg, idx) => {
                if (msg.sender_type === "status") return <StatusBubble key={msg.id} message={msg} />;
                const isLastAIMessage = idx === messages.length - 1 && msg.sender_type === "ai" && !streamingMsgId && !aiTyping;
                return (
                  <MessageBubble
                    key={msg.id}
                    message={msg}
                    ...
                    onSuggestionClick={(text) => { hideAllSuggestions(); sendMessage(text); }}
                    sending={sending}
      ...
      <div className="relative border-t border-black/[0.05] bg-white px-3.5 pb-[max(12px,env(safe-area-inset-bottom))] pt-3 sm:px-4">
        {pendingImage && (
          <div className="mb-2 flex items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-2">
            <img src={pendingImage.previewUrl} alt="Preview" className="h-16 w-16 rounded-xl object-cover" />
            <div className="flex-1">
              <p className="text-[13px] font-medium text-slate-700">Ảnh đã chọn</p>
              <p className="text-[11px] text-slate-500">Nhập mô tả và bấm gửi</p>
            </div>
            <button
        ...
        <div className="flex items-end gap-2">
          ...
          <div className="relative flex-1">
            <input
              ...
              placeholder="Nhắn tin hoặc giữ để nói"
          ...
          {hasInput ? (
            ...
            >
              {sending ? <Loader2 size={18} className="animate-spin" /> : <Send size={18} strokeWidth={2.4} />}
            </button>
          ) : (
            <>
              <button onClick={() => setShowEmoji(true)} className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full text-[#161823] transition active:scale-95">
        ...
        <input ref={cameraInputRef} type="file" accept="image/*" capture="environment" hidden
          onChange={(e) => { const f = e.target.files?.[0]; if (f) handleFileSelect(f); e.target.value = ""; }} />
        <input ref={fileDocInputRef} type="file" hidden
          onChange={(e) => { const f = e.target.files?.[0]; if (f) handleFileSelect(f); e.target.value = ""; }} />
            ...
            <div className="fixed bottom-0 left-0 right-0 z-50 rounded-t-[24px] bg-white pb-[max(20px,env(safe-area-inset-bottom))] pt-2 shadow-2xl">
              ...
              <div className="px-6">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => { setShowFileMenu(false); fileInputRef.current?.click(); }} className="text-sm font-semibold text-blue-500">
                    Tất cả ảnh
                  </button>
                </div>
                <div className="mt-6 grid grid-cols-3 gap-4">
                  ...
                  >
                    ...
                    <span className="text-[13px] font-medium text-slate-700">Album</span>
                  </button>

                  <button
                    onClick={() => { setShowFileMenu(false); fileDocInputRef.current?.click(); }}
                    className="flex flex-col items-center gap-3"
                  >
                    <div className="flex h-20 w-20 items-center justify-center rounded-2xl bg-slate-100 text-slate-700">
                      <Paperclip size={32} strokeWidth={1.8} />
                    </div>
                    <span className="text-[13px] font-medium text-slate-700">Tệp</span>
                  </button>
                </div>
              </div>
            </div>
        ...
        {showEmoji && (
          <>
            ...
            <div className="absolute bottom-20 right-4 z-50 overflow-hidden rounded-lg border border-slate-200 shadow-2xl">
              <EmojiPicker onEmojiClick={(e) => setInput((prev) => prev + e.emoji)} theme="light" height={350} width={300} />
            </div>
          </>
        )}
      </div>
...
function MessageBubble({ message, streamingText, isLastAIMessage, onSuggestionClick, sending, onShowLogin, hiddenSuggestionIds = [], onHideSuggestions }) {
  ...
  if (isSystem) {
    return (
      <div className="flex justify-center">
        <div className="rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs text-slate-500">{message.message}</div>
      </div>
    );
  }
      ...
      <div className="flex justify-end">
        <div className="relative max-w-[78%]">
          ...
          <div className="rounded-[20px] rounded-tr-[6px] bg-[#FFE5EC] px-4 py-2.5 shadow-[0_2px_8px_rgba(254,44,85,0.08)]">
            {message.image_url && (
              <img src={message.image_url} alt="User upload" className="mb-2 max-h-72 w-full rounded-[14px] object-cover" loading="lazy" />
            )}
            {message.message && (
              <p className="whitespace-pre-wrap break-words text-[14px] leading-6 text-[#161823]">{message.message}</p>
            )}
          </div>
        </div>
      </div>
  ...
  const hasSuggestions = isLastAIMessage && message.suggestions?.length > 0 && !isStreaming && !isSuggestionHidden;
  const hasActions = message.actions && Array.isArray(message.actions) && message.actions.length > 0;
  const showLoginButton = isLastAIMessage && !isStreaming && !isSuggestionHidden && shouldShowLoginButton(message.message);
      ...
      <div className="max-w-[82%]">
        <div className="relative">
          ...
          <div className="rounded-[20px] rounded-tl-[6px] bg-white px-4 py-3 shadow-[0_2px_8px_rgba(0,0,0,0.06)]">
            {message.image_url && (
              <img src={message.image_url} alt="Message" className="mb-2 max-h-72 w-full rounded-[14px] object-cover" loading="lazy" />
            )}
            <p className="whitespace-pre-wrap break-words text-[14px] leading-6 text-[#161823]">
              {displayText}
              {isStreaming && <span className="ml-0.5 inline-block h-4 w-[2px] animate-pulse bg-slate-400 align-middle" />}
            </p>
          </div>
        </div>
            ...
            {isAI && (
              <div className="flex items-center gap-0.5">
                <button
                  ...
                  className={`flex h-6 w-6 items-center justify-center rounded-full transition ${feedback === "like" ? "bg-sky-50 text-sky-600" : "text-slate-400 hover:bg-slate-100 hover:text-slate-600"}`}
                >
                  <ThumbsUp size={12} strokeWidth={2.2} />
                </button>
                <button
                  onClick={() => setFeedback(feedback === "dislike" ? null : "dislike")}
                  className={`flex h-6 w-6 items-center justify-center rounded-full transition ${feedback === "dislike" ? "bg-rose-50 text-rose-600" : "text-slate-400 hover:bg-slate-100 hover:text-slate-600"}`}
                >
        ...
        {hasActions && !isStreaming && (
          <div className="mt-2 grid grid-cols-2 gap-2">
            {message.actions.map((action, idx) => (
              ...
              >
                <span className="line-clamp-2 text-[12.5px] font-bold leading-tight text-[#161823]">{action.label}</span>
                <ArrowRight size={14} className="shrink-0 text-[#FE2C55]" strokeWidth={2.6} />
              </button>
            ))}
          </div>
        )}
