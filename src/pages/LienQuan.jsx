...
export default function LienQuan() {
  ...
  const createOrder = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("create-lienquan-order", {
        body: {
    ...
    <div className="min-h-screen bg-[#F5F7FB] pb-24 text-slate-900">
      ...
      <main className="mx-auto w-full max-w-4xl px-4 py-6">
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-black tracking-tight text-slate-900">
              Nạp Liên Quân Mobile
            </h1>
            <p className="text-xs text-slate-500">Chọn mệnh giá và nhập ID</p>
          </div>
        </div>
        ...
        <div className="mb-6 flex items-center gap-2">
          ...
          <StepDot active={step === "username"} number={2} label="Nhập ID" />
    ...
    >
      <div className="relative shrink-0">
        <img
          ...
          onError={(e) => {
            e.currentTarget.src = "https://rwglwovohbyqmbbzdvdj.supabase.co/storage/v1/object/public/game_logos/lienquan.png";
          }}
        />
        {active && (
      ...
      <div className="min-w-0 flex-1">
        <p className="text-base font-black text-slate-900">
          {pkg.quanhuy.toLocaleString("vi-VN")} Quân Huy
        </p>
  ...
  const GUIDE_IMAGES = [
    {
      ...
      title: "Bước 1: Mở game Liên Quân Mobile",
    },
    {
      step: 2,
    ...
    },
    {
      ...
      title: "Bước 3: Copy ID người chơi",
    },
  ];
    ...
    <div className="mx-auto max-w-2xl space-y-4">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <img
                ...
                alt={`${selectedPackage.quanhuy} QH`}
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={onCheck}
            className="flex shrink-0 items-center gap-2 rounded-xl bg-gradient-to-r from-sky-400 to-blue-600 px-4 py-3 text-sm font-bold text-white shadow-md shadow-sky-500/30 transition hover:brightness-110"
          >
            <Search size={18} />
...
function PaymentSection({ order, onBack, onPaid }) {
  ...
  const handleCoinPayment = async () => {
    ...
    setProcessing(true);
    try {
      const { data: paymentResult, error: paymentError } = await supabase.rpc("pay_lienquan_order_with_coins", {
        p_order_id: order.id,
      });
