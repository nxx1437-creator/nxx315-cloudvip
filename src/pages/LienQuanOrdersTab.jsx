...
export default function LienQuanOrdersTab() {
  ...
  const fetchOrders = async (silent = false) => {
    ...
    try {
      ...
      if (fetchError) {
        console.error("FETCH LIENQUAN ORDERS ERROR:", fetchError);
        setError(
          `Không thể tải danh sách đơn Liên Quân: ${
      ...
      console.error("FETCH LIENQUAN ORDERS EXCEPTION:", err);
      setError(
        `Không thể tải danh sách đơn Liên Quân: ${
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          // Chỉ xử lý đơn Liên Quân (package_id bắt đầu lq-)
          const row = payload.new || payload.old;
          const isLienQuan = String(row?.package_id || "")
            .toLowerCase()
            .startsWith("lq-");

          if (!isLienQuan) return;
      ...
      .subscribe((status) => {
        if (status === "CHANNEL_ERROR") {
          console.error("Realtime Liên Quân orders channel error");
        }
      });
  ...
  const updateOrderStatus = async (
    ...
    try {
      ...
      if (updateError) {
        console.error("UPDATE LIENQUAN ORDERS ERROR:", updateError);
      ...
      console.error("UPDATE LIENQUAN ORDERS EXCEPTION:", err);
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-16">
        ...
        <span className="ml-2">Đang tải đơn Liên Quân...</span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <h2 className="text-xl font-bold">Đơn Liên Quân</h2>
          <p className="text-sm text-gray-500">
            Quản lý đơn nạp Quân Huy Liên Quân Mobile.
          </p>
        </div>
      ...
      <div className="flex flex-col gap-3 md:flex-row">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Tìm mã đơn, UID, Quân Huy..."
      ...
      {filteredOrders.length === 0 ? (
        <div className="rounded-2xl border bg-white py-16 text-center text-gray-500">
          Không có đơn Liên Quân phù hợp.
        </div>
      ) : (
                    ...
                    <div className="grid gap-x-8 gap-y-1 text-sm text-gray-600 md:grid-cols-2">
                      ...
                      <div>
                        ...
                        <span className="font-semibold text-gray-900">
                          {Number(order.quanhuy || 0).toLocaleString("vi-VN")}{" "}
                          QH
                        </span>
                      </div>
      ...
      {rejectOrder && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-5 shadow-xl">
            <h3 className="text-lg font-bold">Từ chối đơn Liên Quân</h3>
