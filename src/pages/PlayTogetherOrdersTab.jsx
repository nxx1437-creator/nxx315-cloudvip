...
export default function PlayTogetherOrdersTab() {
  ...
  const fetchOrders = async (silent = false) => {
    ...
    try {
      ...
      if (fetchError) {
        console.error("FETCH PLAYTOGETHER ORDERS ERROR:", fetchError);
        setError(
          `Không thể tải danh sách đơn Play Together: ${
      ...
      console.error("FETCH PLAYTOGETHER ORDERS EXCEPTION:", err);
      setError(
        `Không thể tải danh sách đơn Play Together: ${
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel("admin-playtogether-orders")
      .on(
        "postgres_changes",
        {
          event: "*",
        ...
        (payload) => {
          ...
          const isPlayTogether = row?.pt_uid != null;

          if (!isPlayTogether) return;
      ...
      .subscribe((status) => {
        if (status === "CHANNEL_ERROR") {
          console.error("Realtime Play Together orders channel error");
        }
      });
  ...
  const updateOrderStatus = async (
    ...
    try {
      ...
      if (updateError) {
        console.error("UPDATE PLAYTOGETHER ORDERS ERROR:", updateError);
        setError(
      ...
      console.error("UPDATE PLAYTOGETHER ORDERS EXCEPTION:", err);
      setError(
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-16">
        ...
        <span className="ml-2">Đang tải đơn Play Together...</span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <h2 className="text-xl font-bold">Đơn Play Together</h2>
          <p className="text-sm text-gray-500">
            Quản lý đơn nạp Thỏi Vàng Play Together.
          </p>
        </div>
      ...
      <div className="flex flex-col gap-3 md:flex-row">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Tìm mã đơn, UID, Thỏi Vàng..."
      ...
      {filteredOrders.length === 0 ? (
        <div className="rounded-2xl border bg-white py-16 text-center text-gray-500">
          Không có đơn Play Together phù hợp.
        </div>
      ) : (
                    ...
                    <div className="grid gap-x-8 gap-y-1 text-sm text-gray-600 md:grid-cols-2">
                      ...
                      <div>
                        ...
                        <span className="font-semibold text-gray-900">
                          {Number(order.pt_gold || 0).toLocaleString("vi-VN")} Thỏi
                        </span>
                      </div>
      ...
      {rejectOrder && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-5 shadow-xl">
            <h3 className="text-lg font-bold">Từ chối đơn Play Together</h3>
