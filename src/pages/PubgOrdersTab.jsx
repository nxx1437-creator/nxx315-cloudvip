...
export default function PubgOrdersTab() {
  ...
  const fetchOrders = async (silent = false) => {
    ...
    try {
      const { data, error: fetchError } = await supabase
        .from("orders")
        .select("*")
        .not("pubg_uid", "is", null)
        .order("created_at", { ascending: false });

      if (fetchError) {
        console.error("FETCH PUBG ORDERS ERROR:", fetchError);
        setError(
          `Không thể tải danh sách đơn PUBG: ${
      ...
      console.error("FETCH PUBG ORDERS EXCEPTION:", err);
      setError(
        `Không thể tải danh sách đơn PUBG: ${
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          const isPubg = row?.pubg_uid != null;
      ...
      .subscribe((status) => {
        if (status === "CHANNEL_ERROR") {
          console.error("Realtime PUBG orders channel error");
        }
      });
  ...
  const updateOrderStatus = async (
    ...
    try {
      ...
      if (updateError) {
        console.error("UPDATE PUBG ORDERS ERROR:", updateError);
        setError(
      ...
      console.error("UPDATE PUBG ORDERS EXCEPTION:", err);
      setError(
  ...
  const filteredOrders = useMemo(() => {
    ...
    return orders.filter((order) => {
      ...
      return [
        order.order_code,
        String(order.pubg_uid || ""),
        String(order.pubg_uc || ""),
        order.package_id,
      ]
        .filter(Boolean)
        .some((value) =>
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-16">
        ...
        <span className="ml-2">Đang tải đơn PUBG Mobile...</span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <h2 className="text-xl font-bold">Đơn PUBG Mobile</h2>
          <p className="text-sm text-gray-500">
            Quản lý đơn nạp UC PUBG Mobile VN.
          </p>
        </div>
      ...
      <div className="flex flex-col gap-3 md:flex-row">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Tìm mã đơn, Character ID, UC..."
            className="w-full rounded-xl border py-2.5 pl-10 pr-3 outline-none focus:border-yellow-500"
          />
        </div>
        ...
        <select
          ...
          className="rounded-xl border px-3 py-2.5 outline-none focus:border-yellow-500"
        >
      ...
      {filteredOrders.length === 0 ? (
        ...
        <div className="space-y-3">
          {filteredOrders.map((order) => {
            ...
            const totalUc =
              Number(order.pubg_uc || 0) + Number(order.pubg_bonus || 0);
                    ...
                    <div className="grid gap-x-8 gap-y-1 text-sm text-gray-600 md:grid-cols-2">
                      ...
                      <div>
                        ...
                        <span className="font-semibold text-gray-900">
                          {Number(order.pubg_uc || 0).toLocaleString("vi-VN")} UC
                        </span>
                      </div>
                      ...
                      {Number(order.pubg_bonus || 0) > 0 && (
                        <div>
                          ...
                          <span className="font-semibold text-emerald-600">
                            + {Number(order.pubg_bonus).toLocaleString("vi-VN")} UC
                          </span>
                        </div>
                      )}
                      ...
                      <div>
                        ...
                        <span className="font-semibold text-blue-600">
                          {totalUc.toLocaleString("vi-VN")} UC
                        </span>
                      </div>
      ...
      {rejectOrder && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-5 shadow-xl">
            <h3 className="text-lg font-bold">Từ chối đơn PUBG Mobile</h3>
