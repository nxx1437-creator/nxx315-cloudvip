...
export default function PubgOrdersTab() {
  ...
  const fetchOrders = async () => {
    ...
    if (!error) {
      ...
      console.error("Fetch PUBG orders error:", error);
    }
  ...
  const handleDelivered = async (order) => {
    ...
    if (!error) {
      await notifyTelegram(
        `✅ Đơn PUBG #${order.order_code || order.id} đã giao.\nUID: ${order.pubg_uid}`
      );
      await fetchOrders();
      setDetailOrder(null);
    }
  ...
  const handleReject = async () => {
    ...
    if (!error) {
      await notifyTelegram(
        `❌ Đơn PUBG #${rejectOrder.order_code || rejectOrder.id} bị từ chối.\nLý do: ${rejectReason.trim() || "Không có lý do"}`
      );
      await fetchOrders();
      setRejectOrder(null);
      setRejectReason("");
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-2xl border bg-white p-4 space-y-3">
        <div className="relative">
          ...
          <input
            ...
            placeholder="Tìm mã đơn, Character ID..."
            className="w-full rounded-xl border border-slate-200 py-3 pl-10 pr-4 outline-none transition focus:border-yellow-500"
          />
        </div>
            ...
            <table className="w-full min-w-[900px] text-sm">
              <thead className="border-b bg-slate-50">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left">Gói UC</th>
                  ...
                  <tr key={order.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        {order.pubg_uid && (
                          <button
                            onClick={() => copyText(order.pubg_uid)}
      ...
      {detailOrder && (
        <Modal
          title={`Đơn PUBG #${detailOrder.order_code || detailOrder.id}`}
          onClose={() => setDetailOrder(null)}
        >
          <div className="space-y-3">
            <InfoBox
              label="Character ID"
              value={detailOrder.pubg_uid || "-"}
            />
            <InfoBox
              label="UC gốc"
              value={`${Number(detailOrder.pubg_uc || 0).toLocaleString("vi-VN")} UC`}
            />
            {Number(detailOrder.pubg_bonus || 0) > 0 && (
              <InfoBox
                label="Bonus lần đầu"
                value={`+ ${Number(detailOrder.pubg_bonus).toLocaleString("vi-VN")} UC`}
              />
            )}
            <InfoBox
              label="Tổng nhận"
              value={`${(
                Number(detailOrder.pubg_uc || 0) +
                Number(detailOrder.pubg_bonus || 0)
              ).toLocaleString("vi-VN")} UC`}
            />
            <InfoBox
              label="Số tiền"
