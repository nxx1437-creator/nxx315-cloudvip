...
function hoursUntilMidnight() {
  ...
  return Math.max(
    1,
    Math.round((midnight - now) / 1000 / 60 / 60)
  );
}
...
export default function Tasks() {
  ...
  const isBlocked = Boolean(profile?.is_flagged && !isAdmin);
  ...
  const totalRemaining = tasks.reduce(
    (sum, task) => sum + (task.remainingToday || 0),
    0
  );

  const availableCount = tasks.filter(
    (task) => task.remainingToday > 0
  ).length;
  ...
  const handleStart = async (task) => {
    ...
    if (isBlocked) {
      alert(
        "🚫 Tài khoản của bạn đang bị hạn chế, vui lòng liên hệ email nxx315hub@gmail.com để được hỗ trợ!"
      );
      return;
    }
    ...
    try {
      ...
      if (!data?.shortUrl) {
        alert("Không nhận được liên kết nhiệm vụ. Vui lòng thử lại.");
        return;
      }
      ...
      alert("Không thể mở nhiệm vụ. Vui lòng thử lại sau.");
    } finally {
    ...
    <div className="min-h-screen bg-gradient-to-b from-sky-50 via-white to-white pb-24 font-[Be_Vietnam_Pro]">
      ...
      <main className="mx-auto max-w-md space-y-4 px-4 py-5">
        ...
        <div className="rounded-3xl border border-sky-100 bg-gradient-to-br from-sky-200 via-sky-50 to-white p-5 shadow-lg shadow-sky-100">
          ...
          {/* ADMIN / BLOCK STATUS */}
          {isAdmin && (
            <div className="mt-3 rounded-xl bg-purple-100 px-3 py-2 text-xs font-semibold text-purple-700">
              👑 Admin - Miễn kiểm tra
            </div>
          )}
          ...
          {isBlocked && (
            <div className="mt-3 rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-xs font-semibold text-rose-700">
              🚫 Tài khoản đang bị hạn chế làm nhiệm vụ
            </div>
          )}
            ...
            const progressPct =
              task.daily_limit > 0
                ? Math.min(
                    100,
                    Math.round(
                      (task.completedToday / task.daily_limit) * 100
                    )
                  )
                : 0;
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {task.logo_url ? (
                        ...
                        <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-slate-900 text-xs font-bold text-white">
                          {String(task.provider || "??").slice(0, 2)}
                        </div>
                      )}
