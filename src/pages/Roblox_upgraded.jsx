...
export default function Roblox() {
  ...
  return (
    <div className="min-h-screen bg-[#f7f7f9] pb-24 text-slate-900">
      <TopHeader />
  ...
  <div className="mb-4 flex items-center gap-3">
    <button
      ...
      className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white text-slate-700 shadow-sm ring-1 ring-slate-200 transition hover:bg-slate-50"
    >
      <ArrowLeft size={18} />
    </button>
    <div>
      <h1 className="text-lg font-black tracking-tight text-slate-900 sm:text-xl">Nạp game</h1>
      <p className="text-xs text-slate-500">Nạp Robux nhanh chóng và an toàn</p>
    </div>
  </div>

  {/* Banner giống layout Funtap - chỉ cần đổi ROBLOX_BANNER_URL ở đầu file */}
  <section className="mb-5 overflow-hidden rounded-[26px] bg-white shadow-sm ring-1 ring-slate-200">
    ...
    <div className="p-4 sm:p-5">
      <div className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-3 py-3 shadow-sm">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-sm font-black text-slate-900">{GAME_INFO.name}</p>
          <p className="mt-0.5 text-xs text-slate-500">Server: {GAME_INFO.server}</p>
        </div>
  ...
  {/* 3 bước */}
  <div className="mb-5 rounded-2xl bg-white px-2 py-4 shadow-sm ring-1 ring-slate-200 sm:px-5">
    <div className="flex items-start">
      <ProgressStep
        active={step === "package" || step === "username"}
        done={step === "payment"}
        number={1}
        label="Xác nhận ID"
      />
      <div className={`mt-4 h-1 flex-1 rounded-full ${step !== "package" ? "bg-indigo-600" : "bg-slate-200"}`} />
      <ProgressStep
        active={step === "payment"}
        done={false}
        number={2}
      ...
      />
      <div className={`mt-4 h-1 flex-1 rounded-full ${step === "payment" ? "bg-indigo-600" : "bg-slate-200"}`} />
      <ProgressStep active={false} done={false} number={3} label="Thanh toán" />
    </div>
  </div>
...
function ProgressStep({ active, done, number, label }) {
  return (
    <div className="flex w-[78px] shrink-0 flex-col items-center text-center sm:w-[125px]">
      <div
        className={`flex h-9 w-9 items-center justify-center rounded-full text-sm font-black ${
          active || done
            ? "bg-[#3430a8] text-white shadow-md shadow-indigo-500/20"
            : "bg-slate-200 text-slate-500"
        }`}
      >
        {done ? <CheckCircle2 size={18} /> : number}
      </div>
      <span className={`mt-2 text-[10px] font-bold leading-4 sm:text-xs ${
        active ? "text-[#3430a8]" : "text-slate-500"
      }`}>
        {label}
      </span>
    </div>
...
function UsernameAndPackageSection({
  ...
  return (
    <div className="space-y-4">
      <section className="rounded-[26px] bg-white p-5 shadow-sm ring-1 ring-slate-200 sm:p-7">
        <div className="mb-5">
          <h2 className="text-xl font-black tracking-tight text-slate-900">Nhập ID</h2>
          <p className="mt-1 text-xs text-slate-500">
            Nhập username Roblox để hệ thống xác nhận đúng tài khoản.
          </p>
        </div>

        <label className="mb-2 block text-sm font-bold text-slate-700">Username Roblox</label>
        ...
        <div className="flex gap-2">
          <input
            ...
            placeholder="Nhập User ID / Username"
            className="min-w-0 flex-1 rounded-2xl border border-slate-200 bg-white px-4 py-4 text-sm font-medium text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-blue-500 focus:ring-4 focus:ring-blue-100"
          />
          <button
            onClick={onCheck}
            disabled={checkingUser || !username.trim()}
            className="flex shrink-0 items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-indigo-600 to-blue-600 px-4 py-4 text-sm font-black text-white shadow-md shadow-blue-500/20 transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-50 sm:px-5"
          >
            {checkingUser ? <Loader2 size={18} className="animate-spin" /> : <Search size={18} />}
            <span className="hidden sm:inline">{checkingUser ? "Đang kiểm tra..." : "Xác nhận"}</span>
          </button>
        </div>
        ...
        <div className="mt-5 flex gap-3 rounded-2xl border border-blue-100 bg-blue-50 p-3.5">
          <ShieldCheck size={18} className="mt-0.5 shrink-0 text-blue-600" />
          <div>
            <p className="text-xs font-black text-blue-900">Kiểm tra trước khi nạp</p>
            <p className="mt-1 text-[11px] leading-5 text-blue-700">
              Hãy kiểm tra đúng username. Shop sẽ xử lý theo tài khoản đã xác nhận.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="rounded-[26px] bg-white p-5 shadow-sm ring-1 ring-slate-200 sm:p-7">
        <div className="mb-4">
          <h2 className="text-lg font-black text-slate-900">Chọn gói Robux</h2>
          <p className="mt-1 text-xs text-slate-500">Chọn gói bạn muốn nạp.</p>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl">
            <div className="flex items-start justify-between">
              <h3 className="text-base font-black text-slate-900">Hướng dẫn lấy Tên tài khoản Roblox</h3>
              <button
            ...
            <p className="mt-3 text-sm leading-6 text-slate-600">
              Vui lòng cung cấp <strong className="text-slate-900">Tên tài khoản (@username)</strong> chính xác.
            </p>
            ...
            <button
              onClick={() => setShowGuide(false)}
              className="mt-5 w-full rounded-xl bg-gradient-to-r from-indigo-600 to-blue-600 py-3 text-sm font-bold text-white"
            >
              Đã hiểu
            </button>
          </div>
