...
const PACKAGES = [
  ...
  { id: "pubg-60", amount: 22000, uc: 60, bonus: 28, bonusPercent: 47, image: `${SUPABASE_STORAGE}/uc.png` },
  { id: "pubg-140", amount: 55000, uc: 140, bonus: 68, bonusPercent: 49, image: `${SUPABASE_STORAGE}/uc.png` },
  { id: "pubg-300", amount: 110000, uc: 300, bonus: 129, bonusPercent: 43, image: `${SUPABASE_STORAGE}/uc.png` },
  { id: "pubg-600", amount: 220000, uc: 600, bonus: 284, bonusPercent: 47, image: `${SUPABASE_STORAGE}/uc.png` },
  { id: "pubg-1500", amount: 550000, uc: 1500, bonus: 759, bonusPercent: 51, image: `${SUPABASE_STORAGE}/uc.png` },
  { id: "pubg-3000", amount: 1100000, uc: 3000, bonus: 1969, bonusPercent: 66, image: `${SUPABASE_STORAGE}/uc.png` },
  { id: "pubg-6000", amount: 2200000, uc: 6000, bonus: 4347, bonusPercent: 72, image: `${SUPABASE_STORAGE}/uc.png` },
  { id: "pubg-12000", amount: 4400000, uc: 12000, bonus: 6392, bonusPercent: 53, image: `${SUPABASE_STORAGE}/uc.png` },
  { id: "pubg-18000", amount: 6600000, uc: 18000, bonus: 9048, bonusPercent: 50, image: `${SUPABASE_STORAGE}/uc.png` },
];
...
export default function PubgMobile() {
  ...
  const continueToPayment = async () => {
    if (!selectedPackage) {
      alert("Vui lòng chọn gói UC.");
      return;
    }
    if (!uid.trim()) {
      alert("Vui lòng nhập Character ID PUBG Mobile.");
      return;
    }

    if (!/^\d+$/.test(uid.trim())) {
      alert("Character ID chỉ được chứa số.");
      return;
    }
    if (uid.trim().length < 8 || uid.trim().length > 12) {
      alert("Character ID không hợp lệ (phải từ 8-12 chữ số).");
      return;
    }
    ...
    try {
      ...
      if (error) {
        ...
        try {
          if (error.context) {
            errorStatus = error.context.status || 0;
          ...
          console.warn("Cannot parse error body:", parseErr);
        }

        // 👇 NẾU LÀ 409 (trùng UID pending) → MỞ MODAL
        if (errorStatus === 409 || errorMessage.includes("Bạn đã có đơn chưa thanh toán")) {
          const orderCodeMatch = errorMessage.match(/Mã:\s*([A-Z0-9]+)/i);
          setPendingConflict({
            message: errorMessage,
            orderCode: orderCodeMatch?.[1] || null,
          });
          return;
        }
      ...
      console.error("Create PUBG order:", error);

      const msg = error?.message || "Không thể tạo đơn. Hãy thử lại sau.";

      if (msg.includes("Vui lòng chờ")) {
        alert(`⏱️ ${msg}`);
      } else if (msg.includes("quá nhanh")) {
        alert(`🚫 ${msg}`);
      } else {
        alert(msg);
      }
  ...
  const handleContinuePendingOrder = async () => {
    ...
    try {
      ...
      // Lấy đơn pending mới nhất cho UID này
      const { data: pendingOrder, error: fetchErr } = await supabase
        .from("orders")
        .select("*")
        .eq("user_id", user.id)
        .eq("game", "pubg")
        .eq("pubg_uid", uid.trim())
        .eq("status", "pending")
      ...
      // Nhảy tới trang thanh toán của đơn đó
  ...
  const handleCancelPendingOrder = async () => {
    const ok = window.confirm(
      "Bạn chắc chắn muốn HỦY đơn cũ? Đơn này sẽ không thể thanh toán được nữa."
    );
    if (!ok) return;
    ...
    try {
      ...
      setPendingConflict(null);
      alert("✅ Đã hủy đơn cũ. Bạn có thể tạo đơn mới ngay bây giờ.");
    } catch (error) {
      console.error("Cancel pending order error:", error);
      alert(error?.message || "Không thể hủy đơn. Vui lòng thử lại.");
    } finally {
      setCreatingOrder(false);
    }
  };
  ...
  const handleCardPayment = async () => {
    ...
    try {
      ...
      console.error("PUBG card payment error:", error);
      setCardResult({
        status: "error",
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] pb-40 text-slate-900">
      ...
      {/* Modal xử lý đơn pending trùng UID */}
      {pendingConflict && (
        <PendingOrderModal
          data={pendingConflict}
          processing={creatingOrder}
          onContinue={handleContinuePendingOrder}
...
function HeroBanner({ onBack }) {
  return (
    <div className="relative overflow-hidden rounded-2xl border border-yellow-100 bg-white shadow-sm">
      ...
      <div className="relative -mt-10 px-5 pb-5">
        <div className="flex items-end gap-3.5">
          <div className="relative shrink-0">
            <div className="absolute -inset-0.5 rounded-2xl bg-gradient-to-br from-yellow-400 to-amber-500 opacity-60 blur-md" />
            <div className="relative h-20 w-20 overflow-hidden rounded-2xl border-4 border-white bg-yellow-50 shadow-lg sm:h-24 sm:w-24">
              <img
                src={PUBG_LOGO}
                alt="PUBG Mobile"
                className="h-full w-full object-cover"
          ...
          <div className="min-w-0 flex-1 pb-1">
            ...
            <p className="mt-0.5 text-xs text-slate-500 sm:text-sm">
              Nạp UC chính hãng · Tự động 24/7
            </p>
          </div>
        </div>

        <div className="mt-4 grid grid-cols-3 gap-2">
          <StatPill icon={Zap} label="Tức thì" color="yellow" />
          <StatPill icon={ShieldCheck} label="Bảo mật" color="emerald" />
          <StatPill icon={TrendingUp} label="Chính hãng" color="slate" />
        </div>
      </div>
    </div>
  );
    ...
    <div className="space-y-4">
      <div className="flex items-start gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-4">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-sm font-bold text-amber-900">
            Kiểm tra kỹ Character ID trước khi nạp
          </p>
          <p className="mt-1 text-xs leading-5 text-amber-800">
            UC sẽ nạp vào ID đã xác nhận. Sai ID không hoàn tiền.
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between px-1">
        <div className="flex items-center gap-2.5">
          <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-yellow-400 to-amber-500 text-white shadow-md shadow-yellow-200">
            <Crosshair size={16} />
          </div>
          <div>
            <h2 className="text-base font-black text-slate-900">Chọn gói UC</h2>
      ...
      <div className="pt-2">
        <button
          ...
          className="group relative flex h-14 w-full items-center justify-center gap-2 overflow-hidden rounded-2xl bg-gradient-to-r from-yellow-500 to-amber-600 text-sm font-black text-white shadow-lg shadow-yellow-300/50 transition hover:shadow-yellow-400/60 disabled:cursor-not-allowed disabled:opacity-40 disabled:shadow-none"
        >
  ...
  return (
    <button
      ...
      className={`group relative overflow-hidden rounded-2xl border-2 bg-white p-3 text-left transition-all duration-200 ${
        active
          ? "border-yellow-500 bg-yellow-50/40 shadow-lg shadow-yellow-200/60 -translate-y-1"
          : "border-slate-200 hover:-translate-y-1 hover:border-yellow-300 hover:shadow-lg"
      }`}
    >
      {active && (
        <div className="absolute right-2.5 top-2.5 z-10 flex h-5 w-5 items-center justify-center rounded-full bg-yellow-500 text-white shadow-md">
...
function UidStep({
  ...
  const GUIDE_IMAGES = [
    ...
    {
      step: 2,
      url: `${SUPABASE_STORAGE}/pubg-guide-2.png`,
      title: "Vào Hồ sơ → Copy Character ID",
    },
  ];
    ...
    <div className="mx-auto max-w-2xl space-y-4">
      {selectedPackage && (
        <div className="flex items-center gap-3 rounded-2xl border border-yellow-200 bg-yellow-50 p-4">
          <div className="h-14 w-14 shrink-0 overflow-hidden rounded-xl border border-yellow-100 bg-white">
            <img
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-0.5 text-base font-black text-slate-900">
              {selectedPackage.uc.toLocaleString("vi-VN")} UC
              {selectedPackage.bonus > 0 && (
                <span className="text-emerald-500">
                  {" "}+ {selectedPackage.bonus.toLocaleString("vi-VN")}
                </span>
              )}
            </p>
          </div>
          <p className="shrink-0 text-base font-black text-yellow-600">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
        <div className="mb-5 flex items-center gap-3">
          <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-gradient-to-br from-yellow-400 to-amber-500 shadow-md shadow-yellow-200">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">
              Nhập Character ID để nhận UC
            </p>
          </div>
        </div>

        <label className="mb-2 block text-[11px] font-bold uppercase tracking-widest text-yellow-600">
          Character ID
        </label>
        ...
        <div className="relative">
          ...
          <input
            ...
            inputMode="numeric"
            placeholder="Ví dụ: 5123456789"
            className="w-full rounded-xl border border-slate-200 bg-slate-50 py-4 pl-11 pr-4 text-sm font-bold text-slate-800 outline-none transition placeholder:font-medium placeholder:text-slate-300 focus:border-yellow-400 focus:bg-white focus:ring-4 focus:ring-yellow-100"
          />
        </div>

        <button
          onClick={() => setShowGuide(true)}
          className="mt-3 inline-flex items-center gap-1.5 text-xs font-semibold text-yellow-600 transition hover:text-yellow-700 hover:underline"
        >
          <Info size={13} />
          Hướng dẫn lấy Character ID
        </button>

        <div className="mt-5 flex gap-3 rounded-xl border border-yellow-100 bg-yellow-50/50 p-4">
          <ShieldCheck size={16} className="mt-0.5 shrink-0 text-yellow-600" />
          <div>
            <p className="text-xs font-bold text-yellow-800">
              Kiểm tra chính xác trước khi thanh toán
            </p>
            <p className="mt-1 text-[11px] leading-5 text-yellow-700">
              Character ID gồm 8-12 chữ số. UC sẽ nạp theo ID đã xác nhận — sai
              ID không hoàn tiền.
            </p>
          </div>
        </div>
        ...
        <div className="mt-6 grid grid-cols-2 gap-3">
          ...
          <button
            ...
            className="group relative flex h-12 items-center justify-center gap-2 overflow-hidden rounded-xl bg-gradient-to-r from-yellow-500 to-amber-600 text-sm font-black text-white shadow-md shadow-yellow-200 transition hover:shadow-yellow-300/60 disabled:cursor-not-allowed disabled:opacity-40"
          >
            <div className="absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-white/30 to-transparent transition-transform duration-700 group-hover:translate-x-full" />
            {creatingOrder ? (
              <>
      ...
      {showGuide && (
        <GuideModal onClose={() => setShowGuide(false)} steps={GUIDE_IMAGES} />
      )}
    </div>
  );
}
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 backdrop-blur-sm sm:items-center sm:p-4">
      <div className="relative flex max-h-[90vh] w-full max-w-md flex-col overflow-hidden rounded-t-3xl bg-white shadow-2xl sm:rounded-2xl">
          ...
          {steps.map((item) => (
            <div key={item.step}>
              <div className="mb-2 flex items-center gap-2.5">
                <span className="flex h-7 w-7 items-center justify-center rounded-xl bg-gradient-to-br from-yellow-400 to-amber-500 text-[11px] font-black text-white shadow-md shadow-yellow-200">
                  {item.step}
                </span>
        ...
        <div className="border-t border-slate-100 p-5">
          <button
            onClick={onClose}
            className="h-12 w-full rounded-xl bg-gradient-to-r from-yellow-500 to-amber-600 text-sm font-black text-white shadow-md shadow-yellow-200 transition hover:shadow-yellow-300/60"
          >
            Đã hiểu
          </button>
        </div>
...
function PendingOrderModal({ data, processing, onContinue, onCancel, onClose }) {\n  return (\n    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">\n      <div className="relative w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl">\n        <div className="relative border-b border-amber-100 bg-gradient-to-br from-amber-50 to-orange-50 p-5">\n          <div className="flex items-start gap-3">\n            <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-amber-400 to-orange-500 shadow-md shadow-amber-200">\n              <AlertTriangle size={20} className="text-white" />\n            </div>\n            <div className="min-w-0 flex-1">\n              <h3 className="text-base font-black text-slate-900">\n                Bạn có đơn chưa thanh toán\n              </h3>\n              <p className="mt-0.5 text-xs text-slate-500">\n                Vui lòng chọn hành động bên dưới\n              </p>\n            </div>\n            <button\n              onClick={onClose}\n              disabled={processing}\n              className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-slate-400 transition hover:bg-white/60 disabled:opacity-40"\n            >\n              <X size={16} />\n            </button>\n          </div>\n        </div>\n\n        <div className="space-y-4 p-5">\n          {data.orderCode && (\n            <div className="rounded-xl border border-amber-200 bg-amber-50 p-3.5">\n              <p className="text-[11px] font-bold uppercase tracking-widest text-amber-600">\n                Mã đơn cũ\n              </p>\n              <p className="mt-1 font-mono text-base font-black text-amber-700">\n                {data.orderCode}\n              </p>\n            </div>\n          )}\n\n          <p className="text-sm leading-6 text-slate-600">\n            Bạn đã có đơn nạp cho UID này đang chờ thanh toán. Bạn có thể{" "}\n            <b className="text-slate-900">tiếp tục thanh toán</b> đơn đó, hoặc{" "}\n            <b className="text-slate-900">hủy đơn cũ</b> để tạo đơn mới.\n          </p>\n\n          <div className="space-y-2.5 pt-1">\n            <button\n              onClick={onContinue}\n              disabled={processing}\n              className="group relative flex h-12 w-full items-center justify-center gap-2 overflow-hidden rounded-xl bg-gradient-to-r from-yellow-500 to-amber-600 text-sm font-black text-white shadow-md shadow-yellow-200 transition hover:shadow-yellow-300/60 disabled:opacity-50"\n            >\n              <div className="absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-white/30 to-transparent transition-transform duration-700 group-hover:translate-x-full" />\n              {processing ? (\n                <>\n                  <Loader2 size={16} className="relative animate-spin" />\n                  <span className="relative">Đang xử lý...</span>\n                </>\n              ) : (\n                <>\n                  <CreditCard size={16} className="relative" />\n                  <span className="relative">Tiếp tục thanh toán</span>\n                </>\n              )}\n            </button>\n\n            <button\n              onClick={onCancel}\n              disabled={processing}\n              className="flex h-12 w-full items-center justify-center gap-2 rounded-xl border-2 border-rose-200 bg-rose-50 text-sm font-bold text-rose-600 transition hover:bg-rose-100 disabled:opacity-50"\n            >\n              <XCircle size={16} />\n              Hủy đơn cũ & tạo đơn mới\n            </button>\n          </div>\n        </div>\n      </div>\n    </div>\n  );\n}\n
function PaymentStep({
  ...
  if (!order) {
    return (
      <div className="rounded-2xl border border-slate-200 bg-white p-8 text-center shadow-sm">
        ...
        <button
          onClick={onBack}
          className="mt-5 rounded-xl bg-gradient-to-r from-yellow-500 to-amber-600 px-5 py-3 text-sm font-bold text-white shadow-md shadow-yellow-200"
        >
          Quay lại
        </button>
      </div>
  ...
  const transferContent = `NAP PUBG ${order.order_code} ${order.pubg_uid} ${order.pubg_uc}UC`;
  const coinBalance = Number(profile?.coins || 0);
  const requiredCoins = Number(order.amount || 0);
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-4">
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-100 bg-gradient-to-br from-yellow-50 to-amber-50 p-5">
          ...
          <div className="mt-3 inline-flex items-center gap-1.5 rounded-full border border-yellow-200 bg-white px-3 py-1.5 text-[11px] font-bold text-slate-600">
            Mã đơn:{" "}
            <span className="font-mono text-yellow-700">{order.order_code}</span>
          </div>
        </div>
        ...
        <div className="space-y-4 p-4 sm:p-5">
          ...
          {method === "coin" && (
            <div className="space-y-3">
              ...
              <button
                ...
                className="group relative h-12 w-full overflow-hidden rounded-xl bg-gradient-to-r from-yellow-500 to-amber-600 text-sm font-bold text-white shadow-md shadow-yellow-200 transition hover:shadow-yellow-300/60 disabled:cursor-not-allowed disabled:opacity-40 disabled:shadow-none"
              >
                <div className="absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-white/30 to-transparent transition-transform duration-700 group-hover:translate-x-full" />
          ...
          {method === "bank" && (
            <div className="space-y-3">
              ...
              {order.status === "paid" ? (
                ...
                <button
                  ...
                  className="group relative h-12 w-full overflow-hidden rounded-xl bg-gradient-to-r from-yellow-500 to-amber-600 text-sm font-bold text-white shadow-md shadow-yellow-200 transition hover:shadow-yellow-300/60"
                >
                  <div className="absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-white/30 to-transparent transition-transform duration-700 group-hover:translate-x-full" />
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-3 gap-1.5 sm:grid-cols-5">
                {cardTypes.map((type) => (
                  ...
                  >
                    ...
                    <p className="mt-0.5 text-xs font-black text-yellow-600">
                      {cardDiscounts[type]}%
                    </p>
                  </div>
                ))}
              ...
              {cards.map((card, index) => {
                ...
                return (
                  ...
                  >
                    <div className="mb-3 flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <span className="flex h-6 w-6 items-center justify-center rounded-lg bg-gradient-to-br from-yellow-400 to-amber-500 text-[11px] font-black text-white shadow-sm">
                          {index + 1}
                        </span>
                    ...
                    <div className="space-y-3">
                      <div>
                        <label className="mb-1.5 block text-[11px] font-bold uppercase tracking-widest text-yellow-600">
                          Loại thẻ
                        </label>
                        <select
                          value={card.type}
                          ...
                          className="h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm font-semibold text-slate-700 outline-none transition focus:border-yellow-400 focus:ring-2 focus:ring-yellow-100"
                        >
                          {cardTypes.map((t) => (
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          ...
                          <input
                            ...
                            className="h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm font-semibold text-slate-700 outline-none transition placeholder:font-normal placeholder:text-slate-300 focus:border-yellow-400 focus:ring-2 focus:ring-yellow-100"
                          />
                        </div>
                        <div>
                          <label className="mb-1.5 block text-[11px] font-bold uppercase tracking-widest text-yellow-600">
              ...
              <button
                onClick={addCard}
                className="h-11 w-full rounded-xl border-2 border-dashed border-yellow-200 text-sm font-bold text-yellow-600 transition hover:border-yellow-400 hover:bg-yellow-50"
              >
                + Thêm thẻ
              </button>
              ...
              <button
                ...
                className="group relative flex h-12 w-full items-center justify-center gap-2 overflow-hidden rounded-xl bg-gradient-to-r from-yellow-500 to-amber-600 text-sm font-bold text-white shadow-md shadow-yellow-200 transition hover:shadow-yellow-300/60 disabled:opacity-50"
              >
                <div className="absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-white/30 to-transparent transition-transform duration-700 group-hover:translate-x-full" />
                {cardChecking ? (
                  <>
      ...
      <div className="flex min-w-0 items-center gap-2">
        ...
        {(copy || copyValue) && (
          <button
            type="button"
            onClick={handleCopy}
            className="shrink-0 rounded-lg p-1.5 text-slate-400 transition hover:bg-slate-100 hover:text-yellow-600"
            title="Sao chép"
          >
            <Copy size={14} />
          </button>
